    ...
    if(data->speed > 0) { /* still alive */
      ...
      if(pV->exp_radius < EXP_RADIUS_MAX)
				...
				if (gSettingsCache.team_mode) {
					...
					/* Respawn one eliminated player on the other team if any */
					int respawnIdx = -1;
					...
					if (respawnIdx >= 0) {
						/* respawn at a safe random location */
						float border = 2.0f;
						float g = (float)game2->rules.grid_size;
						int tries = 30;
						float rx = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (g - 2*border);
						float ry = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (g - 2*border);
						while (tries-- > 0) {
							int ok = 1;
							int m;
							...
							if (ok) break;
							rx = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (g - 2*border);
							ry = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (g - 2*border);
						}
						{
							...
							rd->last_dir = rd->dir;
							rd->trailOffset = 0;
							rd->trails[0].vStart.v[0] = rx;
							rd->trails[0].vStart.v[1] = ry;
							rd->trails[0].vDirection.v[0] = 0;
							rd->trails[0].vDirection.v[1] = 0;
							rV->exp_radius = 0;
							initCamera(game->player[respawnIdx].camera, rd, gSettingsCache.camType);
							game->running++;
						}
					}
					/* Check team elimination: if both of a team are gone, other team wins */
					{
						...
						if (team0Alive == 0 || team1Alive == 0) {
							...
							int rep = (winningTeam == 0) ? ((game->player[0].data->speed>0)?0:1)
																					 : ((game->player[2].data->speed>0)?2:3);
							if(mode) {
								createEvent(rep, EVENT_STOP);
								return l;
							}
						}
					}

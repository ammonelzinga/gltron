...
void resetVideoData(void) {
  ...
  for(i = 0; i < game->players; i++) {
    ...
    /* Team mode: override colors so teammates match */
    if (gSettingsCache.team_mode) {
      float team1_model[4] = { 0.2f, 0.2f, 1.0f, 1.0f };
      float team2_model[4] = { 1.0f, 1.0f, 0.2f, 1.0f };
      float team3_model[4] = { 1.0f, 0.2f, 0.2f, 1.0f }; /* red */
      float team1_trail[4] = { 0.2f, 0.2f, 1.0f, 0.6f };
      float team2_trail[4] = { 1.0f, 1.0f, 0.2f, 0.6f };
      float team3_trail[4] = { 1.0f, 0.2f, 0.2f, 0.6f };
      if (i < 2) {
        memcpy(pV->pColorDiffuse, team1_model, sizeof(team1_model));
        memcpy(pV->pColorSpecular, team1_model, sizeof(team1_model));
        memcpy(pV->pColorAlpha, team1_trail, sizeof(team1_trail));
      } else if (i < 4) {
        memcpy(pV->pColorDiffuse, team2_model, sizeof(team2_model));
        memcpy(pV->pColorSpecular, team2_model, sizeof(team2_model));
        memcpy(pV->pColorAlpha, team2_trail, sizeof(team2_trail));
      } else {
        memcpy(pV->pColorDiffuse, team3_model, sizeof(team3_model));
        memcpy(pV->pColorSpecular, team3_model, sizeof(team3_model));
        memcpy(pV->pColorAlpha, team3_trail, sizeof(team3_trail));
      }
    }
...
static void defaultViewportPositions(void) {
  ...
  viewport_content[4] = 4;
  viewport_content[5] = 5;
}

  ...
  for(i = 0; i < vp_max[gViewportType]; i++) {
    ...
    if(d->onScreen == 1) {
      ...
      if (gSettingsCache.show_scores)
				drawScore(p, d);
      drawAcceleratorStatus(p, d);
      /* show team wins in every player's window when team mode enabled */
      drawTeamWins(p, d);
...
void drawCam(Player *p, PlayerVisual* pV) {
  ...
  glLoadIdentity();
  /* use float aspect ratio to avoid integer truncation (which breaks small panes) */
  ...
  /* draw snake pickups in world (upright sprites) */
  {
    extern void snake_drawBugs3D(void);
    snake_drawBugs3D();
  }

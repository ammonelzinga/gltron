...
int processEvent(GameEvent* e) {
	...
	case EVENT_TURN_RIGHT:
		...
		break;
	case EVENT_CRASH:
		if (gSettingsCache.show_crash_messages) {
	...
	case EVENT_STOP:
		...
		if(e->player < PLAYERS && game->player[e->player].ai->active != AI_NONE) {
			...
			/* increment team wins if team mode is enabled */
			if (gSettingsCache.team_mode) {
				int team = (game->winner < 2) ? 0 : (game->winner < 4) ? 1 : 2;
				if (team >= 0 && team < 3) {
					game->team_wins[team]++;
				}
			}
		...
		/* mark finished and route to pause mode */
...
int crashTestPlayers(int i, const segment2 *movement) {
	...
	for(j = 0; j < game->players; j++) {
		...
		for(k = 0; k < game->player[j].data->trailOffset + 1; k++) {
			segment2 *wall = game->player[j].data->trails + k;
			vec2 v;
			float t1, t2;
    ...
    if(data->speed > 0) { /* still alive */
      ...
      t = dt / 100.0f * data->speed * fs;
			
			{
				...
				movement.vDirection.v[1] = t * dirsY[data->dir];
								...
								/* snake: enforce trail cap before collision tests */
								if (gSettingsCache.snake_mode) {
									snake_enforceLength(i);
								}

				crash = crash || crashTestPlayers(i, &movement);
				crash = crash || crashTestWalls(i, &movement);

								/* snake: check bug collision after movement */
								if (gSettingsCache.snake_mode) {
									snake_onPlayerMove(i, &movement);
								}
			}
      ...
      if(pV->exp_radius < EXP_RADIUS_MAX)
				...
				if (gSettingsCache.team_mode) {
					...
					if (threeTeams) {
						int creditTeam = game->player[crashedIdx].data->respawn_credit_team;
						if (creditTeam >= 0) {
							int tStart = (creditTeam == 0) ? 0 : (creditTeam == 1) ? 2 : 4;
							int tEnd = tStart + 2;
							int respawnIdx = -1;
							for (k = tStart; k < tEnd && k < game->players; k++) {
								if (game->player[k].data->speed == SPEED_GONE) { respawnIdx = k; break; }
							}
							if (respawnIdx >= 0) {
								float border = 2.0f;
								float gsz = (float)game2->rules.grid_size;
								int tries = 30;
								float rx = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (gsz - 2*border);
								float ry = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (gsz - 2*border);
								while (tries-- > 0) {
									int ok = 1;
									int m;
									for (m = 0; m < game->players; m++) {
										if (!PLAYER_IS_ACTIVE(game->player + m)) continue;
										float hx, hy; getPositionFromIndex(&hx, &hy, m);
										float dx = hx - rx, dy = hy - ry;
										if (dx*dx + dy*dy < 100.0f) { ok = 0; break; }
									}
									if (ok) break;
									rx = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (gsz - 2*border);
									ry = border + ((float)(trand() & 0xFFFF) / 65535.0f) * (gsz - 2*border);
								}
								Data *rd = game->player[respawnIdx].data;
								PlayerVisual *rV = gPlayerVisuals + respawnIdx;
								rd->speed = game2->rules.speed;
								rd->booster = getSettingf("booster_max");
								rd->boost_enabled = 0;
								rd->trail_height = TRAIL_HEIGHT;
								rd->dir = trand() & 3; rd->last_dir = rd->dir; rd->trailOffset = 0;
								rd->trails[0].vStart.v[0] = rx; rd->trails[0].vStart.v[1] = ry;
								rd->trails[0].vDirection.v[0] = 0; rd->trails[0].vDirection.v[1] = 0;
								rV->exp_radius = 0;
								initCamera(game->player[respawnIdx].camera, rd, gSettingsCache.camType);
								game->running++;
							}
						}
						game->player[crashedIdx].data->respawn_credit_team = -1;
						/* defer winner determination until after all players processed */
					} else {
						int crashedTeam = (crashedIdx < 2) ? 0 : 1;
						int otherTeam = 1 - crashedTeam;
						int respawnIdx = -1;
						int tStart = (otherTeam == 0) ? 0 : 2;
						int tEnd = tStart + 2;
						...
						if (respawnIdx >= 0) {
							...
							Data *rd = game->player[respawnIdx].data;
							PlayerVisual *rV = gPlayerVisuals + respawnIdx;
							rd->speed = game2->rules.speed; rd->booster = getSettingf("booster_max");
							rd->boost_enabled = 0; rd->trail_height = TRAIL_HEIGHT;
							rd->dir = trand() & 3; rd->last_dir = rd->dir; rd->trailOffset = 0;
							rd->trails[0].vStart.v[0] = rx; rd->trails[0].vStart.v[1] = ry;
							rd->trails[0].vDirection.v[0] = 0; rd->trails[0].vDirection.v[1] = 0;
							rV->exp_radius = 0; initCamera(game->player[respawnIdx].camera, rd, gSettingsCache.camType);
							game->running++;
						}
						/* defer 2-team winner determination until after all players processed */
					}
				} else {
					/* defer non-team winner determination until after all players processed */
				}
			}
    }      
  }
	/* Post-loop: perform winner determination after all respawns in this tick */
	if (mode) {
		if (gSettingsCache.team_mode) {
			int k;
			int threeTeams = 0;
			...
			if (threeTeams) {
				int alive[3] = {0,0,0};
				for (k = 0; k < 2; k++) if (game->player[k].data->speed > 0) alive[0]++;
				for (k = 2; k < 4; k++) if (game->player[k].data->speed > 0) alive[1]++;
				for (k = 4; k < 6 && k < game->players; k++) if (game->player[k].data->speed > 0) alive[2]++;
				int teamsAlive = (alive[0] > 0) + (alive[1] > 0) + (alive[2] > 0);
				if (teamsAlive == 1) {
					int winningTeam = (alive[0] > 0) ? 0 : (alive[1] > 0) ? 1 : 2;
					int tStart2 = (winningTeam == 0) ? 0 : (winningTeam == 1) ? 2 : 4;
					int tEnd2 = tStart2 + 2;
					for (k = tStart2; k < tEnd2 && k < game->players; k++) { if (game->player[k].data->speed > 0) { stopRep = k; break; } }
					if (stopRep >= 0) shouldStop = 1;
				}
			} else {
				int team0Alive = 0, team1Alive = 0;
				for (k = 0; k < 2; k++) if (game->player[k].data->speed > 0) team0Alive++;
				for (k = 2; k < 4; k++) if (game->player[k].data->speed > 0) team1Alive++;
				if (team0Alive == 0 || team1Alive == 0) {
					int winningTeam = (team0Alive > 0) ? 0 : 1;
					stopRep = (winningTeam == 0) ? ((game->player[0].data->speed>0)?0:1) : ((game->player[2].data->speed>0)?2:3);
					shouldStop = 1;
				}
			}
		} else {
			if(game->running <= 1) {
				int i2, maxSpeed = SPEED_GONE, winner = -1;
				for(i2 = 0; i2 < game->players; i2++) {
					if(game->player[i2].data->speed >= maxSpeed) {
						winner = i2;
						maxSpeed = game->player[i2].data->speed;
					}
				}
				stopRep = winner;
				shouldStop = 1;
		...
		if (shouldStop && stopRep >= 0) {
			createEvent(stopRep, EVENT_STOP);
			return l;
		}
	}
	return l;
  ...
  int t;
	...
	case GAME_SINGLE_RECORD:
		...
		while(dt > 0) {
			...
			/* snake: spawn/update pickups each physics tick */
			if (gSettingsCache.snake_mode) {
				snake_update(t);
			}
...
void createCrashEvent(int player, int killer, int cause) {
	GameEvent *e;
	List *p = &(game2->events);
